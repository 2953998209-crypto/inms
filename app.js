...
/* ============== 模块隔离上传（核心优化） ============== */
/* 每个模块独立上传、独立校验、独立导入，杜绝跨模块数据污染 */
S.modulePending = { sales: null, salesSummary: null, purchase: null, stock: null };
...
/* 模块校验规则：检查 parseSheet 分类结果是否与期望模块一致 */
function validateModule(detectedKind, expectedModule) {
  if (detectedKind === expectedModule) return true;
  /* 允许的兼容映射（salesSummary 表被 classify 判定为 salesSummary 即合法） */
  return false;
}

/* 模块专属文件处理 */
async function handleFilesForModule(files, moduleType) {
  const out = { sales: [], salesSummary: [], purchase: [], stock: [] };
  const logs = [];
  const logSel = `[data-log="${moduleType}"]`;
  const el = document.querySelector(logSel);
  if (el) { el.innerHTML = ''; el.classList.add('on'); }
  ...
  for (const f of files) {
    try {
      ...
      wb.SheetNames.forEach(sn => {
        ...
        const h = findHeader(rows);
        if (!h) { logs.push(['⚠ 工作表「' + sn + '」未识别到有效表头，已跳过', 'warn']); return; }
        const detectedKind = classify(h.idx, rows, sn, f.name);

        /* 严格校验：检测到的类型必须与目标模块一致 */
        if (detectedKind && detectedKind !== moduleType) {
          logs.push(['✘ 工作表「' + sn + '」被识别为「' + MODULE_NAMES[detectedKind] + '」，不属于当前「' + MODULE_NAMES[moduleType] + '」模块，已拒绝导入', 'err']);
          moduleRejected = true;
          return;
        }
        if (!detectedKind) {
          logs.push(['⚠ 工作表「' + sn + '」无法判定数据类型，已跳过', 'warn']);
          return;
        }

        moduleMatched = true;
        /* 使用 parseSheet 但仅收集目标模块的数据 */
        const tmpOut = { sales: [], salesSummary: [], purchase: [], stock: [] };
        parseSheet(sn, rows, tmpOut, logs, f.name);
        /* 只取目标模块的数据 */
        out[moduleType] = out[moduleType].concat(tmpOut[moduleType] || []);
      });
    } catch (e) { logs.push(['✘ ' + f.name + ' 解析失败：' + e.message, 'err']); }
  }

  /* 后处理：补全分类 */
  out[moduleType].forEach(r => {
    if (!r.cat) {
      const imgCat = guessCatByImage(r.product);
      r.cat = imgCat || (moduleType === 'purchase' || moduleType === 'stock' ? '基础盐' : '未分类');
      r.catSource = r.catSource || (imgCat ? 'image' : 'fallback');
    }
  });

  /* 输出日志 */
  const count = out[moduleType].length;
  logs.forEach(([m, k]) => log(logSel, m, k));
  log(logSel, `\n「${MODULE_NAMES[moduleType]}」模块解析完成：${count} 条数据`, count ? 'ok' : 'warn');

  if (count > 0) {
    S.modulePending[moduleType] = out[moduleType];
    /* 启用对应导入按钮 */
    const btn = document.querySelector(`[data-commit="${moduleType}"]`);
    if (btn) btn.disabled = false;
    const badge = document.querySelector(`[data-count="${moduleType}"]`);
    if (badge) { badge.textContent = count + ' 条'; badge.className = 'badge ok'; }

    /* 自动推断期间 */
    if (!$('#upPeriod').value) {
      const ym = inferPeriod(files);
      if (ym) { $('#upPeriod').value = ym; log(logSel, '🗓 已推断归属期间：' + ym, 'ok'); }
    }
    log(logSel, '➡ 点击【导入】按钮将数据写入期间', 'ok');
  } else {
    S.modulePending[moduleType] = null;
    const btn = document.querySelector(`[data-commit="${moduleType}"]`);
    if (btn) btn.disabled = true;
    const badge = document.querySelector(`[data-count="${moduleType}"]`);
    if (badge) { badge.textContent = '0 条'; badge.className = 'badge'; }
    log(logSel, '未解析到有效数据或数据类型不匹配', 'err');
  }
...
/* 模块专属导入 */
function commitModuleImport(moduleType) {
  const pending = S.modulePending[moduleType];
  if (!pending || !pending.length) return;

  let p = $('#upPeriod').value;
  if (!p) {
    if (moduleType === 'sales' && pending[0] && pending[0].date) {
      const ds = pending.map(r => r.date).filter(Boolean).sort();
  ...
  if (!S.data.periods[p]) S.data.periods[p] = { sales: [], salesSummary: [], purchase: [], stock: [] };
  const cur = S.data.periods[p];

  if (mode === 'replace') {
    /* 仅覆盖当前模块，不影响其他模块数据 */
    cur[moduleType] = pending;
  } else {
    /* 追加合并：仅合并当前模块 */
    cur[moduleType] = (cur[moduleType] || []).concat(pending);
  }
  ...
  const logSel = `[data-log="${moduleType}"]`;
  log(logSel, `✔ 「${MODULE_NAMES[moduleType]}」已导入到期间 ${p}（${pending.length} 条），并已同步到云端`, 'ok');
  log('#parseLog', `✔ ${MODULE_NAMES[moduleType]} → 期间 ${p} · ${pending.length} 条`, 'ok');
  toast(`${MODULE_NAMES[moduleType]}导入成功：` + p, 'ok');
...
function renderModuleOverview() {
  const el = $('#tbModuleOverview');
  if (!el) return;
  const P = cur();
  const modules = [
    { key: 'sales', name: '👤 业务员明细', icon: '👤' },
    { key: 'salesSummary', name: '💰 销售汇总', icon: '💰' },
    { key: 'purchase', name: '📦 购进汇总', icon: '📦' },
    { key: 'stock', name: '📊 现存量', icon: '📊' }
  ];
  const rows = modules.map(m => {
    const data = P[m.key] || [];
    const pending = S.modulePending[m.key];
    return {
      module: m.name,
      count: data.length,
      pending: pending ? pending.length + ' 条待导入' : '—',
      status: data.length ? '已导入' : (pending ? '待确认' : '未导入')
    };
  });
  table('#tbModuleOverview', [
    { t: '模块', k: 'module', f: r => `<b>${r.module}</b>`, html: 1 },
    { t: '已导入条数', n: 1, f: r => r.count },
    { t: '待导入', k: 'pending' },
    { t: '状态', k: 'status' }
  ], rows);
}
...
function renderAll() {
  ...
  try { renderModuleOverview(); } catch (e) { console.error(e); }
}
...
function bind() {
  ...
  /* 模块隔离上传绑定 */
  document.querySelectorAll('.moduleFile').forEach(fi => {
    const mt = fi.dataset.module;
    fi.onchange = e => { if (e.target.files.length) handleFilesForModule([...e.target.files], mt); };
  });
  document.querySelectorAll('.module-drop').forEach(drop => {
    const mt = drop.dataset.module;
    ['dragenter', 'dragover'].forEach(ev => drop.addEventListener(ev, e => { e.preventDefault(); drop.classList.add('hot'); }));
    ['dragleave', 'drop'].forEach(ev => drop.addEventListener(ev, e => { e.preventDefault(); drop.classList.remove('hot'); }));
    drop.addEventListener('drop', e => { if (e.dataTransfer.files.length) handleFilesForModule([...e.dataTransfer.files], mt); });
  });
  document.querySelectorAll('[data-commit]').forEach(btn => {
    btn.onclick = () => commitModuleImport(btn.dataset.commit);
  });
